...
import org.aphreet.c3.snippet.vocabularies.VocabularySection
...
class Boot extends Bootable {
  private val sections: List[Section] =
    List(BaseSection, UsersSection, GroupsSection, CategoriesSection, NotificationsSection, ApproveSection, VocabularySection)
    ...
    def sitemap() = SiteMap(
      ...
      Menu("admin", "Admin") / "admin" >> LocGroup("admin_menus") >> isSuperAdmin submenus {
        List(Menu("categories", "Categories") / "admin" / "categories" submenus {
          ...
          Menu("vocabularies", "Vocabularies") / "admin" / "vocabularies" submenus {
            VocabularySection.menus: _*
          })
      },
